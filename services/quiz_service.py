...
async def submit_quiz_answers(
    ...
    if not correct_answers_map or not questions_details_map: print(f"Error: Quiz session {quiz_session_id} data not found (user submit)."); clear_quiz_session(quiz_session_id); return None
    total_questions_in_session = len(correct_answers_map)
    if total_questions_in_session == 0: print(f"Warning: No questions for session {quiz_session_id}."); clear_quiz_session(quiz_session_id); return QuizResult(total_questions=0, correct_answers_count=0, score_percentage=0.0, analysis=[], incorrect_questions=[])
    correct_count = 0; results_by_category: Dict[str, Dict] = {}; session_categories = set(); incorrect_questions_details: List[IncorrectQuestionDetail] = []
    for q_id, q_detail in questions_details_map.items():
        ...
        correct_ans_text = correct_answers_map.get(q_id) # Ini sudah di-decode
        question_text = q_detail.get('question','') # Ini sudah di-decode dari questions_details_map
        difficulty_text = q_detail.get('difficulty','unknown') # Ini sudah di-decode
        
        if user_ans_obj:
            is_correct = (correct_ans_text is not None and user_ans_obj.user_answer == correct_ans_text)
            if is_correct: correct_count += 1; results_by_category[category_name]["correct"] += 1
            else: incorrect_questions_details.append(IncorrectQuestionDetail(question_id=q_id, question_text=question_text, user_answer=user_ans_obj.user_answer, correct_answer=correct_ans_text or "N/A", category_name=category_name, difficulty=difficulty_text ))
        else: incorrect_questions_details.append(IncorrectQuestionDetail(question_id=q_id, question_text=question_text, user_answer="Tidak dijawab", correct_answer=correct_ans_text or "N/A", category_name=category_name, difficulty=difficulty_text))
    overall_score = round((correct_count / total_questions_in_session) * 100, 2) if total_questions_in_session > 0 else 0.0
    ...
    quiz_result_obj = QuizResult(total_questions=total_questions_in_session, correct_answers_count=correct_count, score_percentage=overall_score, analysis=analysis_list, incorrect_questions=incorrect_questions_details)
    try:
        categories_played_str = ", ".join(sorted(list(session_categories))) if session_categories else None
        new_attempt = QuizAttempt(user_id=user.id,score=overall_score,total_questions=total_questions_in_session,correct_answers=correct_count,categories_played=categories_played_str)
        db.add(new_attempt); await db.commit(); await db.refresh(new_attempt); print(f"Quiz attempt {new_attempt.id} saved for user {user.id}")
    except Exception as e: await db.rollback(); print(f"ERROR saving quiz attempt: {e}")
    clear_quiz_session(quiz_session_id); return quiz_result_obj
